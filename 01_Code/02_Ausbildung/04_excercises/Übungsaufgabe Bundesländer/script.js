  ...
  for (let i = 0; i < bundeslaender.length; i++) {
    ...
    content.innerHTML += `
        <div class="card">
            ...
            <input id="input${i}"><button onclick="addComment(${i})">Kommentar hinzufügen</button>
        </div>
        `; //name ist hier der key
...
function addComment(index) {
  let input = document.getElementById(`input${index}`); // Wieso man hier nicht "value" schreiben darf, verstehe ich nicht.
