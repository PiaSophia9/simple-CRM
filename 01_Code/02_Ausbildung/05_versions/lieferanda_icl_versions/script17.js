...
function renderDish(dishesObject, i) {
  for (let j = 0; j < dishesObject.items.length; j++) {
    let itemsObject = dishesObject.items[j]; // is the same as: let itemsObject = dishes[i].items[j]
...
function addToShoppingCart(itemsObjectName, i, j) {
  // remove placeholder from shoppingcart:
  document.getElementById("shoppingCartPlaceholder").classList.add("d_none");
  // copy item from array "dishes" into array shoppingCart
  addObjectToArrayShoppingCart(itemsObjectName, i, j);
  // render and generate shoppingCart
  renderDishIntoShoppingCart();
  // generate and show total price of all dishes in shoppingCart:
  // displayTotalSum();
}
...
function generateDishInShoppingCart(i) {
  return /*html*/ `
          <div class="section_shopping_cart__container_item">
            ...
            <p id="price${i}" class="section_shopping_cart__price">${shoppingCart[i].shoppingCartPrice}€</p>
          </div>
