...
function generateDishInShoppingCart(i) {
  return /*html*/ `
          <div class="section_shopping_cart__container_item">
            ...
            <p id="price${i}" class="section_shopping_cart__price">${shoppingCart[i].shoppingCartPrice.toFixed(2)}€</p>
          </div>
...
function increaseDishSum(i) {
  ...
  document.getElementById(`price${i}`).innerHTML = `${sumDish.toFixed(2)}€`; //Wird richtig angezeigt. Danach passiert aber noch etwas! Sprung zu Zeile 166, wo der ganze Warenkorb nochmal - ohne toFixed - gerendert wird.
  displayTotalSum();
}
