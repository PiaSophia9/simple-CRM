...
function renderDish(dishesObject, i) {
  for (let j = 0; j < dishesObject.items.length; j++) {
    ...
    let formattedPrice = itemsObject.price.toFixed(2).replace(".", ",");
    document.getElementById(`menuesContainer${i}`).innerHTML += generateDish(itemsObject, itemsObjectName, i, j, formattedPrice);
  }
}

function generateDish(itemsObject, itemsObjectName, i, j, formattedPrice) {
  return /*html*/ `
        <div class="section_menus__dishes">
            ...
            <p class="section_menus__price"><b id="dishPriceID${i}">${formattedPrice}€</b></p>
        </div>
      `;
}
